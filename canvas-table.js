...
(function ($) {
  $.fn.canvasTable = function (_options) {
    ...
    const initArea = async (_image) => {
      ...
      return new Promise((resolve, reject) => {
        ...
        image.onload = () => {
          ...
          image.xTranslate = -(xDistance / 2);
          image.yTranslate = -(yDistance / 2);

          image.scale = _image.scale || 1;
          image.rotate = _image.rotate || 0;

          image.xPos = area.xPos;
          image.yPos = area.yPos;
    ...
    const changeImage = (fromAreaId, toAreaId) => {
      ...
      fromArea.image.xTranslate = -(fromArea.image.width - toArea.width) / 2;
      fromArea.image.yTranslate = -(fromArea.image.height - toArea.height) / 2;

      toArea.image.xTranslate = -(toArea.image.width - fromArea.width) / 2;
      toArea.image.yTranslate = -(toArea.image.height - fromArea.height) / 2;

      fromArea.image.xPos = toArea.xPos;
      fromArea.image.yPos = toArea.yPos;

      toArea.image.xPos = fromArea.xPos;
      toArea.image.yPos = fromArea.yPos;
